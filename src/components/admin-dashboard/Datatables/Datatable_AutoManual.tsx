...
import { adminDashboardAutoManualCols } from "@/utils/datatable/columns/AdminDatatableColumns";
...
const Datatable_AutoManual = ({
  ...
  const getTaskStatusData = async (value: string) => {
    ...
    const url = `${process.env.report_api_url}/dashboard/manualautobreakidlelist`;
    const successCallback = (
      ResponseData: {
        TotalCount: number;
        ManualAutoListFilters: null;
        ManualAutoList: ListDashboard[] | [];
      },
      error: boolean,
      ResponseStatus: string
    ) => {
      if (ResponseStatus.toLowerCase() === "success" && error === false) {
        setData(ResponseData.ManualAutoList);
        setTableDataCount(ResponseData.TotalCount);
        onHandleExport(ResponseData.ManualAutoList.length > 0 ? true : false);
        setLoading(false);
      } else {
        setLoading(false);
      }
  ...
  return (
    <div>
      ...
      <ThemeProvider theme={getMuiTheme()}>
        <MUIDataTable
          data={data}
          columns={adminDashboardAutoManualCols}
          title={undefined}
...
export default Datatable_AutoManual;
